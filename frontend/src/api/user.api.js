...
export const resetSendOtp = async (email) => {
    const res = await axiosInstance.post("/api/v1/auth/resetpassword/sendOtp", {email});
    return res.data;
}

export const resetVerifyOtp = async (email, otp) => {
    const res = await axiosInstance.post("/api/v1/auth/resetpassword/verifyOtp", {email, otp});
    return res.data;
}

export const resetPasswordSubmit = async (email, password) => {
    const res = await axiosInstance.post("/api/v1/auth/resetpassword/submit", {email, password});
    return res.data;
}
...
export const getAllUserQr = async () => {
    const { data } = await axiosInstance.post("/api/v1/user/qrs");
    return data;
}
...
export const transactionFetch = async (sessionId) => {
    // console.log("frontend api".sessionId);
